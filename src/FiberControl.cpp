...
namespace wxm {
	...
	// thread_local std::weak_ptr<Fiber> FiberControl::runningFiber(nullptr);
	// thread_local std::weak_ptr<Fiber> FiberControl::mainFiber(nullptr);
	// thread_local std::weak_ptr<Fiber> FiberControl::schedulerFiber(nullptr);
	thread_local std::weak_ptr<Fiber> FiberControl::runningFiber;
	thread_local std::weak_ptr<Fiber> FiberControl::mainFiber;
	thread_local std::weak_ptr<Fiber> FiberControl::schedulerFiber;
	...
	void FiberControl::first_create_fiber() {
		...
		assert(FiberControl::runningFiber.lock() == fiber);
		assert(FiberControl::mainFiber.lock() == fiber);
		assert(FiberControl::schedulerFiber.lock() == fiber);
	...
	std::shared_ptr<Fiber> FiberControl::create_fiber(std::function<void()> _cb, size_t _stacksize, bool _run_in_scheduler) {
		if (!FiberControl::runningFiber.lock()) {
			first_create_fiber();
		}
	...
	std::shared_ptr<Fiber> FiberControl::get_running_fiber() { // 首先运行该函数会创建主协程
		...
		return FiberControl::runningFiber.lock();
	}


	std::shared_ptr<Fiber> FiberControl::get_main_fiber() {
		if (!FiberControl::mainFiber.lock()) {
			first_create_fiber();
		}
		return FiberControl::mainFiber.lock();
	}


	std::shared_ptr<Fiber> FiberControl::get_scheduler_fiber() {
		if (!FiberControl::schedulerFiber.lock()) {
			first_create_fiber();
		}
		return FiberControl::schedulerFiber.lock();
	}
